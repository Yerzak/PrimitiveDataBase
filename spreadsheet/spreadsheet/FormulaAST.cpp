...
namespace ASTImpl {
    ...
    public:
        ...
        virtual std::optional<double> Evaluate(SheetFunction& args) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            std::optional<double> Evaluate(SheetFunction& args) const override {
                auto lhs_res = lhs_->Evaluate(args);
                auto rhs_res = rhs_->Evaluate(args);
                if (GetPrecedence() == EP_DIV) {
                    if (std::isfinite(rhs_res.value()) && std::isfinite(lhs_res.value()) && std::isfinite(lhs_res.value() / rhs_res.value())) {
                        return { lhs_res.value() / rhs_res.value() };
                    }
                    else {
                        throw FormulaError(FormulaError::Category::Arithmetic);
                    }
                }
                else if (GetPrecedence() == EP_ADD && std::isfinite(lhs_res.value()) && std::isfinite(rhs_res.value()) && std::isfinite(lhs_res.value() + rhs_res.value())) {
                    return { lhs_res.value() + rhs_res.value() };
                }
                else if (GetPrecedence() == EP_SUB && std::isfinite(lhs_res.value()) && std::isfinite(rhs_res.value()) && std::isfinite(lhs_res.value() - rhs_res.value())) {
                    return { lhs_res.value() - rhs_res.value() };
                }
                else if (GetPrecedence() == EP_MUL && std::isfinite(lhs_res.value()) && std::isfinite(rhs_res.value()) && std::isfinite(lhs_res.value() * rhs_res.value())) {
                    return { lhs_res.value() * rhs_res.value() };
                }
                throw FormulaError(FormulaError::Category::Arithmetic);
            }
        ...
        public:
            ...
            std::optional<double> Evaluate(SheetFunction& args) const override {
                auto result = operand_->Evaluate(args);
                return type_ == Type::UnaryMinus ? (-1 * result.value()) : result;
            }
        ...
        public:
            ...
            std::optional<double> Evaluate(SheetFunction& args) const override {
                /*Чтобы передать параметр — скажем, ссылку на таблицу внутрь метода,
                можно использовать объекты-функции. Например, функцию,
                которая по индексу ячейки возвращает её значение в конкретной таблице.*/
                return args(*cell_);
            }
...
std::optional <double> FormulaAST::Execute(SheetFunction& args) const {
